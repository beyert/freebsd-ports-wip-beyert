--- sshkey.c.orig	2018-11-25 15:34:22 UTC
+++ sshkey.c
...
@@ -669,17 +648,31 @@ sshkey_equal_public(const struct sshkey *a, const stru
 #ifdef WITH_OPENSSL
 	case KEY_RSA1:
 	case KEY_RSA_CERT:
-	case KEY_RSA:
...
+		}
 # ifdef OPENSSL_HAS_ECC
 	case KEY_ECDSA_CERT:
 	case KEY_ECDSA:
@@ -754,15 +747,21 @@ to_blob_buf(const struct sshkey *key, struct sshbuf *b
 			return ret;
 		break;
 #ifdef WITH_OPENSSL
-	case KEY_DSA:
...
+		}
 ...
 # ifdef OPENSSL_HAS_ECC
 	case KEY_ECDSA:
@@ -775,13 +774,18 @@ to_blob_buf(const struct sshkey *key, struct sshbuf *b
 			return ret;
 		break;
 # endif
-	case KEY_RSA:
...
+		}
 ...
 #endif /* WITH_OPENSSL */
 	case KEY_ED25519:
@@ -887,9 +891,14 @@ sshkey_fingerprint_raw(const struct sshkey *k, int dgs
...
+
 		if (nlen < 0 || elen < 0 || nlen >= INT_MAX - elen) {
 			...
 			goto out;
@@ -899,8 +908,8 @@ sshkey_fingerprint_raw(const struct sshkey *k, int dgs
...
+		if (BN_num_bits(n) != (int)bits)
 		...
 		*cpp = ep;
 		retval = 0;
@@ -1425,19 +1444,20 @@ sshkey_format_rsa1(const struct sshkey *key, struct ss
 #ifdef WITH_SSH1
 	u_int bits = 0;
...
@@ -1769,15 +1789,32 @@ sshkey_from_private(const struct sshkey *k, struct ssh
 	switch (k->type) {
 #ifdef WITH_OPENSSL
 	case KEY_DSA:
-	case KEY_DSA_CERT:
...
+			}
 		...
 		break;
 # ifdef OPENSSL_HAS_ECC
@@ -1800,13 +1837,22 @@ sshkey_from_private(const struct sshkey *k, struct ssh
...
+			}
 		...
 		break;
 #endif /* WITH_OPENSSL */
@@ -2004,10 +2050,20 @@ sshkey_from_blob_internal(struct sshbuf *b, struct ssh
...
@@ -2025,12 +2081,34 @@ sshkey_from_blob_internal(struct sshbuf *b, struct ssh
 			ret = SSH_ERR_ALLOC_FAIL;
 			goto out;
 		}
...
@@ -2270,26 +2348,53 @@ sshkey_demote(const struct sshkey *k, struct sshkey **
 			goto fail;
 		/* FALLTHROUGH */
 	case KEY_RSA1:
-	case KEY_RSA:
...
+			}
 		...
 		break;
 	case KEY_ECDSA_CERT:
@@ -2410,12 +2515,17 @@ sshkey_certify(struct sshkey *k, struct sshkey *ca, co
...
+		}
 ...
 # ifdef OPENSSL_HAS_ECC
 	case KEY_ECDSA_CERT:
@@ -2427,10 +2537,15 @@ sshkey_certify(struct sshkey *k, struct sshkey *ca, co
 			goto out;
 		break;
 # endif /* OPENSSL_HAS_ECC */
-	case KEY_RSA_CERT:
...
@@ -2587,43 +2702,65 @@ sshkey_private_serialize(const struct sshkey *key, str
 		goto out;
 	switch (key->type) {
 #ifdef WITH_OPENSSL
-	case KEY_RSA:
...
+		}
 ...
 # ifdef OPENSSL_HAS_ECC
 	case KEY_ECDSA:
@@ -2699,18 +2836,51 @@ sshkey_private_deserialize(struct sshbuf *buf, struct 
 			r = SSH_ERR_ALLOC_FAIL;
 			goto out;
 		}
...
+		}
 ...
 # ifdef OPENSSL_HAS_ECC
 	case KEY_ECDSA:
@@ -2769,24 +2939,84 @@ sshkey_private_deserialize(struct sshbuf *buf, struct 
 			r = SSH_ERR_ALLOC_FAIL;
 			goto out;
 		}
...
+		}
 ...
 #endif /* WITH_OPENSSL */
 	case KEY_ED25519:
@@ -3421,6 +3651,7 @@ sshkey_private_rsa1_to_blob(struct sshkey *key, struct
...
@@ -3447,10 +3678,13 @@ sshkey_private_rsa1_to_blob(struct sshkey *key, struct
...
@@ -3475,9 +3709,9 @@ sshkey_private_rsa1_to_blob(struct sshkey *key, struct
 		goto out;
...
@@ -3668,6 +3909,8 @@ sshkey_parse_private_rsa1(struct sshbuf *blob, const c
 	struct sshcipher_ctx *ciphercontext = NULL;
 	const struct sshcipher *cipher;
...
+	    *iqmp = NULL;
 	...
 	if (keyp != NULL)
 		*keyp = NULL;
@@ -3703,11 +3946,17 @@ sshkey_parse_private_rsa1(struct sshbuf *blob, const c
 		goto out;
...
@@ -3736,15 +3985,33 @@ sshkey_parse_private_rsa1(struct sshbuf *blob, const c
 	}
...
@@ -3817,7 +4084,9 @@ sshkey_parse_private_pem_fileblob(struct sshbuf *blob,
...
@@ -3835,7 +4104,7 @@ sshkey_parse_private_pem_fileblob(struct sshbuf *blob,
 		r = SSH_ERR_LIBCRYPTO_ERROR;
 		goto out;
 	}
...
+	if (EVP_PKEY_id(pk) == EVP_PKEY_RSA &&
 		...
 		if ((prv = sshkey_new(KEY_UNSPEC)) == NULL) {
 			r = SSH_ERR_ALLOC_FAIL;
@@ -3850,7 +4119,7 @@ sshkey_parse_private_pem_fileblob(struct sshbuf *blob,
 			r = SSH_ERR_LIBCRYPTO_ERROR;
 			goto out;
 		}
...
+	} else if (EVP_PKEY_id(pk) == EVP_PKEY_DSA &&
 		...
 		if ((prv = sshkey_new(KEY_UNSPEC)) == NULL) {
 			r = SSH_ERR_ALLOC_FAIL;
@@ -3862,7 +4131,7 @@ sshkey_parse_private_pem_fileblob(struct sshbuf *blob,
