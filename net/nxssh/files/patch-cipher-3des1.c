--- cipher-3des1.c.orig	2018-11-25 15:34:22 UTC
+++ cipher-3des1.c
@@ -44,7 +44,7 @@
  */
...
@@ -65,7 +65,7 @@ ssh1_3des_init(EVP_CIPHER_CTX *ctx, const u_char *key,
 	if (key == NULL)
 		return 1;
 	if (enc == -1)
-		enc = ctx->encrypt;
...
@@ -74,12 +74,19 @@ ssh1_3des_init(EVP_CIPHER_CTX *ctx, const u_char *key,
 		else
 			k1 += 16;
 	}
...
@@ -95,9 +102,9 @@ ssh1_3des_cbc(EVP_CIPHER_CTX *ctx, u_char *dest, const
...
+		EVP_CIPHER_CTX_free(c->k3);
 		...
 		free(c);
 		EVP_CIPHER_CTX_set_app_data(ctx, NULL);
@@ -128,13 +135,13 @@ ssh1_3des_iv(EVP_CIPHER_CTX *evp, int doset, u_char *i
...
@@ -142,17 +149,14 @@ ssh1_3des_iv(EVP_CIPHER_CTX *evp, int doset, u_char *i
 const EVP_CIPHER *
 evp_ssh1_3des(void)
 {
