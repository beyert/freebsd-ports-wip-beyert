--- src/control/midi/portmidi/pm_common/portmidi.c.orig	2022-07-16 11:22:47 UTC
...
@@ -28,15 +61,16 @@ static void prompt_and_exit(void)
 {
...
+    } else if (err == pmHostError) {
          ...
          */
@@ -61,114 +95,298 @@ system implementation of portmidi interface
 */
...
@@ -184,15 +402,15 @@ pm_fns_node pm_none_dictionary = {
     none_write_flush,
     none_synchronize,
     none_open,
-    none_abort, 
...
+{
     ...
     switch(errnum)
@@ -201,48 +419,63 @@ const char *Pm_GetErrorText( PmError errnum ) {
         msg = ""; 
         break;
     case pmHostError:                
...
@@ -250,24 +483,24 @@ void Pm_GetHostErrorText(char * msg, unsigned int len)
 }
...
@@ -275,44 +508,47 @@ PmError Pm_Initialize( void ) {
 }
...
@@ -321,67 +557,80 @@ PmError Pm_Read(PortMidiStream *stream, PmEvent *buffe
...
+    else if(!pm_descriptors[midi->device_id].pub.output)
         ...
         err = pmNoError;
@@ -398,9 +647,21 @@ PmError Pm_Write( PortMidiStream *stream, PmEvent *buf
...
@@ -423,7 +684,7 @@ PmError Pm_Write( PortMidiStream *stream, PmEvent *buf
...
+                /* this should be a realtime message */
                 if (is_real_time(msg)) {
                     if ((err = (*midi->dictionary->write_realtime)(midi, 
                                        &(buffer[i]))) != pmNoError)
@@ -444,16 +705,27 @@ PmError Pm_Write( PortMidiStream *stream, PmEvent *buf
             }
         }
...
+                    if (err != pmNoError) goto error_exit;
                 ...
                 bits += 8;
@@ -465,13 +737,19 @@ PmError Pm_Write( PortMidiStream *stream, PmEvent *buf
         }
     }
...
@@ -481,13 +759,15 @@ PmError Pm_WriteShort( PortMidiStream *stream, long wh
 }
...
+    PmInternal *midi = (PmInternal *) stream;
     /* the next byte in the buffer is represented by an index, bufx, and
        a shift in bits */
     int shift = 0;
@@ -499,198 +779,335 @@ PmError Pm_WriteSysEx(PortMidiStream *stream, PmTimest
...
@@ -698,18 +1115,20 @@ PmError Pm_SetFilter(PortMidiStream *stream, long filt
 }
...
+    else if (!pm_descriptors[midi->device_id].pub.opened)
     ...
     if (err != pmNoError) 
@@ -718,171 +1137,264 @@ PmError Pm_Close( PortMidiStream *stream ) {
     /* close the device */
